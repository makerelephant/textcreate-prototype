...
export type EnvReport = {
  ...
  persistenceMode: "supabase_postgres";
  publicAppUrl: string;
};
...
export function getEnvReport(): EnvReport {
  ...
  if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN || !process.env.TWILIO_PHONE_NUMBER) {
    warnings.push("Missing Twilio credentials (SID, token, or phone number).");
  }
  if (!process.env.OPENAI_API_KEY) warnings.push("OPENAI_API_KEY is missing.");
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    warnings.push("Missing Supabase env (SUPABASE_URL and/or SUPABASE_SERVICE_ROLE_KEY).");
  }
  if (!process.env.SUPABASE_STORAGE_BUCKET) {
    warnings.push("SUPABASE_STORAGE_BUCKET not set; defaulting to 'textcreate-media'.");
  }
  if (publicAppUrl.includes("localhost") || publicAppUrl.includes("127.0.0.1")) {
    warnings.push("Public app URL points to localhost; Twilio/OpenAI cannot reach local URLs.");
  }
  if (signatureValidationDisabled) warnings.push("Twilio signature validation is disabled.");
  if (imageGenerationEnabled && !process.env.OPENAI_API_KEY) {
    warnings.push("Image generation enabled but OPENAI_API_KEY is missing.");
  }
